...
function createGmeme(imgId) {
    return {
        selectedImgId: imgId,
        txts: [createTxt('Seu texto', 150, 70), createTxt('Seu texto', 150, 300)]
    };
}
...
function renderTxtsEditor() {
    var strHtml = gMeme.txts.map(function (txt, idx) {
        ...
        <div class="txt-editor">
                    ...
                    <label for="outline">Largura</label>
                    do Contorno: <input type="number" value="${txt.lineWidth}"  min="0" step="1" data-property="lineWidth" oninput="editTxt(this ,${idx})">
                    ...
                    <label for="shadow">Sombras</label>
...
function newTxtBtnClicked() {
    gMeme.txts.push(createTxt('Nova linha', 150, 150));
    drawCanvas();
    renderTxtsEditor();
}
...
function dlCanvas(eldllink) {
    ...
    dt = dt.replace(/^data:application\/octet-stream/, 'data:application/octet-stream;headers=Content-Disposition%3A%20attachment%3B%20filename=your_meme.png');
